...
object DisputeTransition:
  ...
  private def validateDisputes(
    ...
    for (verdict, verdictIdx) <- disputes.verdicts.zipWithIndex do
      ...
      then return Some(DisputeErrorCode.JudgementsNotSortedUnique)
    ...
    then return Some(DisputeErrorCode.CulpritsNotSortedUnique)
    ...
    then return Some(DisputeErrorCode.FaultsNotSortedUnique)
  ...
  /** Process disputes and update state. Returns tuple of (new state, offenders
    * mark for output) Note: The offenders mark in the output retains processing
    * order (culprits then faults) while the state.psi.offenders list gets
    * sorted offenders added to it.
    */
